...
for (let i = 0; i < runners.length; i++) {
    firstNames.push(runners[i].first_name);
    lastNames.push(runners[i].last_name);

    for (let x = 0; x < firstNames.length; x++) {
        for (let y = 0; y < lastNames.length; y++) {
            fullName.push(firstNames[x] + '-' + lastNames[y]);
...
for (let i = 0; i < firstNames.length; i++) {
    allCaps.push(firstNames[i].toUpperCase());
}
console.log(allCaps);
...
var total = ticketPriceTotal.reduce((total, amount) => total + amount);
console.log(total);
